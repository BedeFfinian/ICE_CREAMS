...
def _get_s2_files_from_safe(input_s2_safe) -> dict:
    ...
    output_files_dict["scene_date"] = int(os.path.basename(input_s2_safe.rstrip("/")).split("_")[2][0:8])
    
    output_files_dict["processing_base_line"] = int(os.path.basename(input_s2_safe.rstrip("/")).split("_")[3][1:4])
...
def _read_s2_data_xarray(input_s2_files, mask_vector_file=None):
    ...
    # Bias removed for images after 2022
#    if input_s2_files["scene_date"] > 20220125:
#        for band in [b01, b02, b03, b04, b05, b06, b07, b08, b08a, b09, b11, b12]:
#            band.data = band.data - 1000
    
    if input_s2_files["processing_base_line"] > 401:
        for band in [b01, b02, b03, b04, b05, b06, b07, b08, b08a, b09, b11, b12]:
            band.data = band.data - 1000
...
def read_s2_safe(input_s2_safe, mask_vector_file=None):
    """
    Function to read S2 data from SAFE file and return as an Dataset with
...
def read_s2_stac(input_s2_name, mask_vector_file=None):
    """
    Function to read S2 data from STAC file and return as an Dataset with
    bands resampled to 10m
    """
    s2_files_dict = _get_s2_files_from_stac(input_s2_name)
    return _read_s2_data_xarray(s2_files_dict, mask_vector_file)
...
def calc_ndfc(s2_data_raw):
    """
    Function to calculate NDFC from S2 data loaded as xarray Dataset For Post-Processing

    Returns xarray DataArray
    """
    red_raw = s2_data_raw["Reflectance_B04"]
    ir_raw = s2_data_raw["Reflectance_B09"]

    ndfc_raw = (ir_raw - red_raw) / (ir_raw + red_raw)
    ndfc_raw.name = "NDFC"
...
def calc_spc(s2_data_raw):
    """
    Function to calculate true NDVI from S2 data loaded as xarray Dataset For Post-Processing
...
def apply_classification(input_xarray, class_model):
    ...
    # Apply model with a batch size of 512
    ...
    class_probs = preds.max(
        axis=1
    )  # These two lines should pull the probability the model gave the majority class
    class_probs_values = (
        class_probs.values
    )  # This line is sometimes needed and othertimes not to get probabilities. (Not sure why!)

    input_df["out_class"] = out_class
    input_df["class_probs"] = class_probs_values

    # We want to do some post processing where the function above called calc_ndfc() is used.
    # this metric will not be used by the model but when the pixel out_class==3 and the ndfc<0.05 we want to change out_class to 9
    ndfc_mask = (input_df["out_class"] == 3) & (input_df["NDFC"] < 0.05)
    input_df["out_class"] = input_df["out_class"].where(~ndfc_mask, other=9)
    input_df["Unknown20"] = input_df["SPC"].where(input_df["out_class"] == 9, other=0)
    ...
    input_df["Unknown20"] = input_df["Unknown20"].where(input_df["Unknown20"]>=20,other=0)
    input_df["Unknown20"] = input_df["Unknown20"].where(input_df["Unknown20"]<=100,other=100)
    
    input_df["SPC20Unknown"] = input_df["Unknown20"] + input_df["SPC20"]
    ...
    input_df["SPC"] = input_df["SPC"].where(
        input_df["Reflectance_B01"] != 0, other=-1
    )
...
def classify_s2_scene(
    ...
    if stac:
        print(f"Searching for {input_s2_safe} in STAC catalogue and loading...")
        s2_data_raw = read_s2_stac(input_s2_safe, mask_vector_file)
    else:
    ...
    ndfc_raw = calc_ndfc(s2_data_raw)
    ...
    s2_data = xarray.merge([s2_data_raw, s2_data_stan, ndwi_raw, ndvi_true_raw,ndfc_raw,spc_raw])
    ...
    if debug:
        ...
        out_class_dataset = out_classified_s2_data[
            ["out_class", "class_probs","SPC","SPC20","SPC20Unknown"]
        ].squeeze(dim="band", drop=True)

    out_class_dataset.assign_attrs(
        {
            "description": "BiCOME Advanced Intertidal Classification",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Apply BiCOME Intertidal Classification to an S2 image"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="STAC mode. Rather than loading S2 data from file, "
        "searches for in Microsoft Planetary Computer STAC catalogue",
    )
